...
7. **문의하기 위젯** — 관람객 화면 오른쪽 아래의 `문의하기` 버튼. 공연·회차 잔여석·본인 예매·환불 안내를 조회 툴로만 답합니다(`POST /api/chat`, 로그인 게이트 밖 공개). 예매나 취소를 대신 하지는 않습니다. AI 키가 없으면 고정 안내 문구로 폴백하고, `/admin`·`/seller`에서는 뜨지 않습니다.
...
- **Vitest** — 순수 로직, Store 구현, API route를 테스트 우선으로 검증합니다. 83개 파일 · 704개 테스트가 CI에서 lint·build와 함께 돕니다.
...
| `pnpm lint` | ESLint |
| `pnpm test` | Vitest 전체 테스트 (83개 파일 · 704개 테스트) |
