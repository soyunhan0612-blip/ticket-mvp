...
- DS는 아이콘 세트를 제공하지 않는다. 아이콘 라이브러리도 의존성에 없다.
- 인라인 SVG, 기본 `h-5 w-5`, `stroke="currentColor"`, `strokeWidth={1.5}`, 24 그리드
- 둥근 배경 아이콘 컨테이너로 감싸지 않으며, 버튼에서는 항상 텍스트 라벨과 함께 사용
- 전부 `chatbot/ui/icons.tsx`에 있다 — 말풍선(런처·도우미 턴), 헤드셋(상담원 턴·연결 버튼),
  정보(안내 턴·상담 상태), 닫기. 문의 위젯 밖에서 쓰는 아이콘은 아직 없다
- **예외 하나**: 문의 위젯 패널 안의 좁은 컨트롤(닫기)에만 아이콘 단독 버튼을 허용한다.
  런처와 상담원 연결은 텍스트 라벨을 유지한다 — 패널 밖이거나 처음 누르는 버튼이다
- 아이콘이 텍스트 라벨을 대신하는 자리에서는 `sr-only` 라벨을 남긴다. `aria-label`보다
  먼저 이것을 쓴다 — 실제 텍스트 노드라 번역·검색·테스트에 그대로 걸린다

---

## 문의 위젯 표면 (`chatbot/ui/ChatWidget.tsx`)

- 패널은 `h-[30rem]` 고정이고 `max-h-[calc(100dvh-5rem)]`로만 줄어든다. 높이를 상한으로만
  잡으면 메시지마다 패널이 위로 자라 입력창 위치가 널뛴다. `vh`가 아니라 `dvh`인 이유는
  모바일 주소창이 `100vh`에 포함되기 때문이다
- 스크롤 컨테이너는 대화가 비어 있어도 마운트해 둔다. 빈 상태와 목록을 갈아끼우면 첫
  메시지에서 재마운트돼 ref가 끊긴다. 그 컨테이너만 `flex-1 min-h-0`이고 헤더·배너·폼은
  전부 `shrink-0`이다
- 손님 턴은 우측 정렬 `bg-ink` 채움, 도우미·상담원 턴은 좌측 평문이다. **좌/우 말풍선을
  양쪽 다 채우지 않는다** — 쓸 수 있는 채움이 `primary`뿐인데 그러면 손님 문장이 화면에서
  가장 강한 요소가 되어 "채운 red는 화면당 하나" 규칙이 깨진다. CSS 변수 색이라
  `bg-primary/10` 같은 옅은 톤도 만들 수 없다(§Tailwind 주의)
- `bg-canvas-soft`는 안내(notice) 턴과 상담 상태 배너 전용이다. 빈 상태나 손님 턴에 쓰면
  세 가지가 같은 표면으로 보인다
- 역할 라벨은 상담원·안내만 눈에 보인다. 손님과 도우미는 정렬·표면이 이미 화자를 말하므로
  `sr-only`로 내린다
- 그림자를 쓰지 않으므로 헤더 하단 헤어라인이 고정 크롬과 대화를 가르는 신호다
