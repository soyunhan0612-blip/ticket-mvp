...
- **Operations를 별도 API가 아니라 기존 admin 게이트 아래에 둔 이유** — 미들웨어의 `isProtectedApiPath`가 게이트하는 경로는 `/api/admin`뿐이고, 이 게이트는 쿠키 외에 `Authorization: Basic` 헤더도 이미 허용한다(README의 심사자용 `curl -u` 때문에 남겨둔 경로다). 그 자리에 두면 n8n 연동에 인증 코드가 한 줄도 필요 없다. 반대로 `/api/ai/*` 옆에 두면 기존 AI 라우트가 게이트 **바깥**에 있는 배치를 그대로 물려받아 매출·재고가 공개된다

**트레이드오프**:
- n8n이 쓰는 자격증명이 `/admin`·`/seller`와 **같은 단일 Basic 자격증명**이다. 역할 분리가 없으므로 유출되면 셀러 공연 등록까지 열린다. 역할을 나누려면 `isProtectedPath`의 경로 판정부터 손대야 하고, 이번 범위에서는 감수하고 회전 정책으로 대신한다
