...
실측(2026-09-13)은 이보다 한 걸음 더 나쁘다. 좌석 2,000개의 마운트 렌더 합계가 43.4 ms → 142.5 ms로 **3.3배 늘었다.** 좌석마다 파생 atom과 구독을 만드는 비용이다. 같은 측정에서 클릭 커밋은 27.1 ms → 0.7 ms로 줄었다. 구조가 바꾼 것은 총량이 아니라 비용이 발생하는 시점이다 — 자세한 값과 조건은 [Perf Measurement](PERF_MEASUREMENT.md#2-실측-결과-2026-09-13).
