...
새 AI 라우트를 만들 때 기본은 게이트 뒤다 — 운영 데이터를 다루면서 공개 배치를 복제하면 매출·재고가 그대로 공개된다. 관람객 챗봇만 예외인데, 익명 손님이 쓰는 창구라 게이트 뒤로 갈 수 없기 때문이다. 대신 내보내는 범위를 **좌석 화면이 이미 공개하는 것**(공연·회차·잔여석)으로 묶었다 (ADR-008).
