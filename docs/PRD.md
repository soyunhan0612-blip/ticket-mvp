...
8. 관람객 문의 챗봇 — 공연·회차·내 예매·환불 안내를 조회 툴로 답하고, AI가 답할 수 없는 문의는 슬랙의 사람에게 넘긴다. 1분 안에 응대가 없으면 자동 안내가 뜨고, 상담원이 슬랙 스레드에 답장하면 챗봇 창에 실린다. 7번과 같은 확장 범위이며 **도메인 데이터는 조회 전용**이다. 실시간 갱신은 3초 폴링이다 — 아래 3대 함정 3번은 그대로 유효하다. 챗봇 모듈은 다른 프로젝트로 폴더째 이식할 수 있게 `src/chatbot/`에 자기 완결적으로 둔다 (ADR-008)
