...
1. **부작용 툴 하나를 허용한다** — `escalate_to_human`. 슬랙에 메시지를 올리고 대화에 대기 상태를 세우는 것이 부작용의 전부다. 좌석·예약 상태를 건드리지 않으므로 "왜 이 좌석이 풀렸는가"를 사후에 설명할 수 없게 만드는 종류의 쓰기가 아니다. 조건 셋을 만족해야 한다.
   ...
   - **동시 1회.** 대기 중인 에스컬레이션이 있으면 저장소가 거부한다. 상담원이 답한 뒤에는 다시 올릴 수 있다
...
3. **관람객 조회 라우트는 게이트 밖에 둔다** — 아래 "엔드포인트 배치"의 예외다. `/api/chat`은 익명 손님이 쓰는 창구라 `/api/admin/**` 아래로 가면 Basic 게이트에 막힌다. 내보내는 범위는 **좌석 화면이 이미 공개하는 것**(공연·회차·좌석 현황 집계)으로 한정하고 매출·판매율 같은 운영 수치는 넣지 않는다
